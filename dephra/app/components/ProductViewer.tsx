...
function Loader() {
  ...
  return (
    <Html center>
      <div className="flex flex-col items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur rounded-lg shadow-lg border border-zinc-100">
        <Loader2 className="w-5 h-5 animate-spin text-blue-600" />
        <span className="text-xs font-semibold text-zinc-600">{progress.toFixed(0)}% loaded</span>
      </div>
    </Html>
  );
}
...
export default function ProductViewer() {
  ...
  const [dpr, setDpr] = useState(1.5); // Start with decent quality, downgrade if needed
          ...
          {view === "2d" ? (
            ...
            >
               <Canvas 
                  ...
                  shadows={false} // Disable expensive internal shadow maps
                >
                  {/* Dynamic performance scaling: Drop quality if FPS dips */}
                  <PerformanceMonitor 
                  ...
                  <Suspense fallback={<Loader />}>
                    ...
                    <group position={[0, -0.5, 0]}>
                       <Model url={modelPath} />
                       {/* Reduced shadow resolution (256) for performance */}
                       <ContactShadows resolution={256} scale={10} blur={2} opacity={0.4} far={10} color="#000000" />
                    </group>
